...
#include <apps/netutils/libcoap/coap_io.h>
...
coap_network_send(struct coap_context_t *context UNUSED_PARAM,
		  ...
		  unsigned char *data,
		  size_t datalen) {
#if !defined(WITH_CONTIKI) && !defined(WITH_NUTTX)
  struct coap_endpoint_t *ep =
  ...
  return sendto(local_interface->handle, data, datalen, 0,
...
#if defined(WITH_POSIX) || defined(WITH_NUTTX)
...
#endif /* WITH_POSIX */
#ifdef WITH_NUTTX
  coap_address_t *src = &(*packet)->src;
  coap_address_t *dst = &(*packet)->dst;

  if (getsockname(ep->handle, &dst->addr.sa, &dst->size) < 0) {
      dst->addr.sin.sin_family = AF_INET;
      dst->addr.sin.sin_port = 0;
      dst->addr.sin.sin_addr.s_addr = INADDR_ANY;
      dst->size = sizeof(dst->addr.sin);
  }

  len = recvfrom(ep->handle,
                 (*packet)->payload,
                 coap_get_max_packetlength(*packet),
                 0,
                 &src->addr.sa,
                 &src->size);
