...
coap_print_wellknown(coap_context_t *context, unsigned char *buf,
                     size_t *buflen, size_t offset, coap_opt_t *query_filter) {
  return print_wellknown(context, buf, buflen, offset, query_filter);
}
...
#if defined(WITH_POSIX) || defined(WITH_NUTTX) || defined(WITH_LWIP)
#ifdef COAP_RESOURCES_NOHASH
