...
{
/* Silence scan-build analyzer. It thinks that 'i' in last loop
 * can get value of 8, which gives "result of '<<' expression
 * is undefined". */
...
static int ssl_handle_possible_reconnect( mbedtls_ssl_context *ssl )
{
    ...
    ret = ssl_check_dtls_clihlo_cookie(
            ...
            ssl->out_buf, MBEDTLS_SSL_MAX_OUT_CONTENT_LEN, &len );
...
int mbedtls_ssl_write_certificate( mbedtls_ssl_context *ssl )
{
    ...
    {
        n = crt->raw.len;
        if( n > MBEDTLS_SSL_MAX_OUT_CONTENT_LEN - 3 - i )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "certificate too large, %d > %d",
                           i + 3 + n, MBEDTLS_SSL_MAX_OUT_CONTENT_LEN ) );
...
{
    int ret;
    const size_t inlen = MBEDTLS_SSL_BUFFER_LEN;
    const size_t outlen = MBEDTLS_SSL_OUT_BUFFER_LEN;
     ...
     */
    if( ( ssl->in_buf = mbedtls_calloc( 1, inlen ) ) == NULL )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "alloc(%d bytes) failed", inlen ) );
        return( MBEDTLS_ERR_SSL_ALLOC_FAILED );
    }

    if( ( ssl->out_buf = mbedtls_calloc( 1, outlen ) ) == NULL )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "alloc(%d bytes) failed", outlen ) );
                ...
                const unsigned char *psk_identity, size_t psk_identity_len )
{
    ...
    if( ( psk_identity_len >> 16 ) != 0 ||
        psk_identity_len > MBEDTLS_SSL_MAX_OUT_CONTENT_LEN )
    {
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
    }
                           ...
                           const unsigned char *buf, size_t len )
{
    ...
    if( len > MBEDTLS_SSL_MAX_OUT_CONTENT_LEN )
    {
        len = MBEDTLS_SSL_MAX_OUT_CONTENT_LEN;
    }
