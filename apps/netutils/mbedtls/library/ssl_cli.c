...
/*
 * Return 1 is timer is expired, 0 if it isn't, -1 if timer not active.
 */
static int ssl_check_timer( mbedtls_ssl_context *ssl )
{
    ...
    ret = ssl->f_get_timer( ssl->p_timer );
    if( ret == 2 )
    {
    ...
    if( ret < 0 )
    {
        MBEDTLS_SSL_DEBUG_MSG( 3, ( "timer inactive" ) );
        return( -1 );
    }
...
{
    unsigned char *p = buf;
    const unsigned char *end = ssl->out_msg + MBEDTLS_SSL_MAX_OUT_CONTENT_LEN;
    size_t hostname_len;
                                    ...
                                    size_t pms_offset )
{
    ...
    if( offset + len_bytes > MBEDTLS_SSL_MAX_OUT_CONTENT_LEN )
    {
    ...
    if( ( ret = mbedtls_pk_encrypt( &ssl->session_negotiate->peer_cert->pk,
                            ...
                            MBEDTLS_SSL_MAX_OUT_CONTENT_LEN - offset - len_bytes,
...
int mbedtls_ssl_handshake_client_step( mbedtls_ssl_context *ssl )
{
    int ret = 0;
#if defined(MBEDTLS_SSL_PROTO_DTLS)
    int ongoing_hs_flight_from_server = 0;
#endif
    ...
    if ((ssl->state >= MBEDTLS_SSL_SERVER_HELLO &&
      ...
      {
        ongoing_hs_flight_from_server = 1;
      }
#endif
    ...
    if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
        ...
        ongoing_hs_flight_from_server != 0 && ssl_check_timer( ssl ) == 1 )
    {
        /* Prevent slow client processing to trigger retransmission. */
        ssl_set_timer( ssl, ssl->conf->hs_timeout_min );
    }
#endif
