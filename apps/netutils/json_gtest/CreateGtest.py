...
for obj in unique:
    ...
    if (json_object != None):
        ...
        for (path, value) in objwalk(json_object):
            ...
            for p in path:
                if (type(p) == int):
                    print '\t\tASSERT_EQ(cJSON_Array, cJSON_type(child));'
                    print '\t\tchild = cJSON_GetArrayItem(child, ' + str(p) + ');'
                else:
                    print '\t\tASSERT_EQ(cJSON_Object, cJSON_type(child));'
                    print '\t\tchild = cJSON_GetObjectItem(child, "' + c_string(p) + '");'
                print '\t\tASSERT_TRUE(NULL != child);'
            if (value == None):
                print '\t\tASSERT_EQ(cJSON_NULL, cJSON_type(child));'
            elif (type(value) == str or type(value) == unicode):
                print '\t\tASSERT_EQ(cJSON_String, cJSON_type(child));'
                print '\t\tASSERT_STREQ("' + c_string(value) + '", cJSON_string(child));'
            elif (type(value) == int or type(value) == float):
                print '\t\tASSERT_EQ(cJSON_Number, cJSON_type(child));'
                if (math.isnan(value) or math.isinf(value)):
                    print '\t\t// Testing for NaN or Infinity is not implemented'
                else:
                    print '\t\tASSERT_FLOAT_EQ(' + str(value) + ', cJSON_double(child));'
            elif (type(value) == bool):
                if (value):
                    print '\t\tASSERT_EQ(cJSON_True, cJSON_type(child));'
                else:
                    print '\t\tASSERT_EQ(cJSON_False, cJSON_type(child));'
            else:
