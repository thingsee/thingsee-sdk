...
#define COAP_OPT_BLOCK_LAST(opt) \
  (__coap_opt_block_last((opt), COAP_OPT_LENGTH(opt)))

inline static unsigned char *
__coap_opt_block_last(const coap_opt_t *opt, size_t len) {
  static const unsigned char __coap_opt_block_last_nul = 0;
  return len ? (COAP_OPT_VALUE(opt) + (len - 1))
             : (unsigned char*)&__coap_opt_block_last_nul;
}

/** Returns the value of the More-bit of a Block option @p opt. */
#define COAP_OPT_BLOCK_MORE(opt) \
  (__coap_opt_block_more((opt), COAP_OPT_LENGTH(opt)))

inline static int
__coap_opt_block_more(const coap_opt_t *opt, size_t len) {
  return len ? (*__coap_opt_block_last(opt, len) & 0x08) : 0;
}

/** Returns the value of the SZX-field of a Block option @p opt. */
#define COAP_OPT_BLOCK_SZX(opt)  \
  (__coap_opt_block_szx((opt), COAP_OPT_LENGTH(opt)))

inline static int
__coap_opt_block_szx(const coap_opt_t *opt, size_t len) {
  return len ? (*__coap_opt_block_last(opt, len) & 0x07) : 0;
}
