...
static inline void
coap_nuttx_clock_init_impl(void) { }

#define coap_clock_init coap_nuttx_clock_init_impl

static inline void
coap_nuttx_ticks_impl(coap_tick_t *t) {
  struct timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
  *t = ts.tv_sec * COAP_TICKS_PER_SECOND +
       ((uint64_t)ts.tv_nsec * COAP_TICKS_PER_SECOND / (1000 * 1000 * 1000));
}

#define coap_ticks coap_nuttx_ticks_impl

#define clock_offset ((time_t)0)
#endif /* WITH_NUTTX */
